...
PLOT_MARGIN = 0.1 # Add 10% margin around the trajectory extents
PLOT_INITIAL_SCALE = 20.0 # Default scale when trajectory is small
...
def calculate_adaptive_scale_and_origin(points_array, canvas_width, canvas_height, margin=0.1, initial_scale=20.0):
    """Calculates adaptive scale and origin to fit trajectory."""
    if points_array.shape[0] < 2:
        # Not enough points, use default centered origin and initial scale
        origin_u = canvas_width // 2
        origin_v = canvas_height // 2
        return initial_scale, origin_u, origin_v

    min_x, max_x = points_array[:, 0].min(), points_array[:, 0].max()
    min_z, max_z = points_array[:, 2].min(), points_array[:, 2].max()

    world_range_x = max_x - min_x
    world_range_z = max_z - min_z

    # Add margin to the ranges
    world_range_x += world_range_x * margin * 2 # Margin on both sides
    world_range_z += world_range_z * margin * 2

    # Prevent division by zero if range is tiny
    epsilon = 1e-6
    world_range_x = max(world_range_x, epsilon)
    world_range_z = max(world_range_z, epsilon)

    # Calculate required scale for each axis
    scale_x = canvas_width / world_range_x
    scale_z = canvas_height / world_range_z

    # Use the smaller scale to ensure both axes fit
    new_scale = min(scale_x, scale_z)

    # Calculate the center of the trajectory in world coordinates
    center_x = (min_x + max_x) / 2
    center_z = (min_z + max_z) / 2

    # Calculate the origin point (where world 0,0 maps to) on the canvas
    # We want the world center to map to the canvas center
    canvas_center_u = canvas_width / 2
    canvas_center_v = canvas_height / 2

    new_origin_u = int(canvas_center_u - center_x * new_scale)
    new_origin_v = int(canvas_center_v + center_z * new_scale) # Remember +Z world is often -V canvas

    # Handle initial zoom level if path is very small
    if new_scale > initial_scale * 5: # Don't zoom in excessively at start
         new_scale = initial_scale * 5

    # Prevent extremely large scales if range is near zero
    # new_scale = min(new_scale, 1000) # Optional: Limit max zoom
...
def draw_trajectory(canvas, points_array, origin_u, origin_v, scale):
    ...
    # Draw world origin (0,0) marker if visible
    world_origin_u = int(origin_u)
    world_origin_v = int(origin_v)
    h, w = canvas.shape[:2]
    if 0 <= world_origin_u < w and 0 <= world_origin_v < h:
        cv2.circle(canvas, (world_origin_u, world_origin_v), 5, (0, 200, 0), -1) # Green circle at origin
...
# Initialize plot variables with defaults
plot_scale = PLOT_INITIAL_SCALE
plot_origin_u = PLOT_CANVAS_WIDTH // 2
plot_origin_v = PLOT_CANVAS_HEIGHT // 2
...
while True: # Main loop starts here
    ...
    frame_count += 1
    if frame_count % FRAME_SKIP != 0: continue # Frame Skipping
    ...
    # --- Depth Estimation ---
    # (Keep this block largely the same, ensure current_depth_map is set or None)
    ...
    try:
        ...
        # print(f"Frame {frame_count}: Error depth: {e}") # Optional: Debug depth errors
        current_depth_map = None; depth_display_color = np.zeros((PROCESS_HEIGHT, process_width, 3), dtype=np.uint8)
    depth_time = (time.time() - start_time_depth) * 1000

    # --- Feature Detection ---
    keypoints, descriptors = orb.detectAndCompute(frame_gray, None)

    if descriptors is None or current_depth_map is None:
        if descriptors is None: print(f"Frame {frame_count}: No descriptors")
        # Keep previous data if needed for next iteration, but skip pose estimation
        if descriptors is not None:
             prev_frame_gray = frame_gray.copy(); prev_keypoints = keypoints; prev_descriptors = descriptors
        continue # Skip pose estimation if features or depth failed

    frame_display = cv2.drawKeypoints(frame_resized, keypoints, None, color=(0, 255, 0), flags=0)

    # --- Feature Matching and Pose Estimation ---
    # (Keep this block largely the same, ensuring pose_updated flag is set)
    ...
    if prev_frame_gray is not None and prev_descriptors is not None and previous_depth_map is not None:
        ...
        good_matches = []
        try: # Handle potential empty matches list
    ...
    if len(trajectory_points) > 1 : # Check if we have points to calculate scale from
        traj_array = np.array(trajectory_points)
        # Calculate new scale and origin based on *all* points so far
        plot_scale, plot_origin_u, plot_origin_v = calculate_adaptive_scale_and_origin(
            traj_array, PLOT_CANVAS_WIDTH, PLOT_CANVAS_HEIGHT, PLOT_MARGIN, PLOT_INITIAL_SCALE
        )
        # Draw using the newly calculated scale and origin
        draw_trajectory(plot_canvas, traj_array, plot_origin_u, plot_origin_v, plot_scale)
    else:
        # If only the starting point exists, draw just that with initial scale/origin
        draw_trajectory(plot_canvas, np.array(trajectory_points), plot_origin_u, plot_origin_v, plot_scale)
    ...
    info_text2 = f"M: {num_good_matches} Scl: {estimated_scale:.2f} DptT: {depth_time:.0f}" # Shorter text
...
# (Keep this section as before)
try:
    print("Plotting final trajectory...")
    ...
    if trajectory.shape[0] > 1:
        ...
        ax.set_xlabel('X [m] (guessed)'); ax.set_ylabel('Z [m] (guessed)'); ax.set_title('Final Estimated Trajectory')
        ax.grid(True); ax.legend(); ax.axis('equal'); plt.show()
